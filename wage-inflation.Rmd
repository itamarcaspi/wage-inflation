...
df_nest <- df_change %>%
  select(DATE, type, sector_name, dlog_value_y) %>% 
  nest(-sector_name) %>% 
  mutate(data_spread = map(data, ~ spread(.x, type, dlog_value_y))) %>% 
...
The following code chunk performs a two sided Granger causality test and returns a $2\times 1$ vector of test statistics.
...
Next, we plot the results of the bootstrap Granger causality tests. In particular, 
...
df %>% 
  ...
  select(-DATE, -lc_bs, -lc_all, -p_all) %>%
  mutate_all(~ 100*(log(.)-log(lag(. ,8)))) %>% 
  drop_na() %>% 
  cor() %>% 
  corrplot(type = "lower",
...
Gali, Jordi, and Luca Gambetti. "Has the U.S. wage Phillips curve flattened? A semi‐structural exploratio." National Bureau of Economic Research working paper 25476.
...
Hooper Peter, Frederic S. Mishkin and Amir Sufi: Prospects for inflation in a high pressure economy: Is the Phillips curve dead or is it just hibernating?  National Bureau of Economic Research working paper 25792 (2018)
